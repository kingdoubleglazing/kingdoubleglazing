...
import type { SiteSettings, PricingOption, FaqItem, EstimatePage } from '@/sanity/types'
...
export default async function InstantEstimatePage() {
  const [settings, pricingOptions, estimateFaqs, estimatePage] = await Promise.all([
    sanityFetch<SiteSettings>({ query: SITE_SETTINGS_QUERY, tags: ['siteSettings'] }),
    sanityFetch<PricingOption[]>({ query: PRICING_QUERY, tags: ['pricingOption'] }),
    sanityFetch<FaqItem[]>({ query: FAQS_QUERY, params: { group: 'estimate' }, tags: ['faqItem'] }),
    sanityFetch<EstimatePage>({ query: ESTIMATE_PAGE_QUERY, tags: ['estimatePage'] }),
  ])
  ...
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: estimateFaqs.map(item => ({
      '@type': 'Question',
      name: item.q,
    ...
    <>
      ...
      <Suspense fallback={<div className="bg-surface-container-low h-96" />}>
        <GlassComparisonTable
          ...
          secondStoreySurcharge={estimatePage.secondStoreySurcharge}
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-10 py-4">
        <p className="font-sans text-sm text-on-surface leading-relaxed">
          ...
          <a href={settings.phoneHref} className="underline hover:text-primary transition-colors duration-150">
            {settings.phone}
          </a>.
        </p>
      </div>
      ...
      <FAQ
        ...
        items={estimateFaqs.map(f => ({ q: f.q, a: f.a }))}
        emitSchema={false}
      />
