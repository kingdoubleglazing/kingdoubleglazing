...
import type { WarrantyPageQuery, WarrantyPageQueryVariables } from '@/tina/__generated__/types'

type TinaWarrantyResult = { data: WarrantyPageQuery; query: string; variables: WarrantyPageQueryVariables }
...
export function WarrantyPageClient({
  ...
  tinaWarranty: TinaWarrantyResult
  phone: string
  phoneHref: string
  email: string
}) {
  const { data } = useTina(tinaWarranty)
  const page = data.warrantyPage
    ...
    <>
        ...
        <div className="relative max-w-5xl mx-auto px-4">
          ...
          >
            <span data-tina-field={tf(page, 'heroHeadline') || undefined}>
              {page?.heroHeadline ?? 'Our 10-Year'}
            </span>{' '}
            <span
              data-tina-field={tf(page, 'heroHeadlineYellow') || undefined}
              className="text-primary-container"
            >
              {page?.heroHeadlineYellow ?? 'Warranty'}
            </span>
          </h1>
          <p
          ...
          >
            {page?.heroSubtext ?? 'Every installation is backed by a 10-year warranty on glass and workmanship. No conditions, no fine print.'}
          </p>
        </div>
      </section>
                ...
                {(page?.coveredItems ?? [
                  ...
                  return (
                    <li key={c.item ?? i} className="ghost-border p-6 flex items-start gap-4">
                      ...
                      <div>
                        <p
                          data-tina-field={tf(c, 'item') || undefined}
                          className="font-headline text-sm font-semibold uppercase tracking-wide text-on-surface mb-1"
                        >
                          {c.item}
                        </p>
                        <p
                          data-tina-field={tf(c, 'detail') || undefined}
                          className="font-sans text-sm text-on-surface leading-relaxed"
                        >
                          {c.detail}
                        </p>
                      </div>
              ...
              <div className="mt-8 bg-surface-container p-6 ghost-border">
                ...
                <ol className="space-y-2 font-sans text-sm text-on-surface leading-relaxed list-decimal list-inside">
                  {(page?.claimSteps ?? [
                    ...
                    <li
                      key={i}
                      data-tina-field={tf(page, 'claimSteps') || undefined}
                    >
                      {step}
                    </li>
                  ))}
      ...
      <section className="bg-primary-container py-16 md:py-20 overflow-hidden relative">
        ...
        <div className="relative max-w-5xl mx-auto px-4 text-center">
          <h2
            data-tina-field={tf(page, 'ctaHeadline') || undefined}
            className="font-display uppercase leading-[0.88] text-on-primary-fixed mb-4"
            style={{ fontSize: 'clamp(2.5rem, 7vw, 5rem)' }}
          >
            {page?.ctaHeadline ?? 'Ready to Get Your Price?'}
          </h2>
          <p
            data-tina-field={tf(page, 'ctaSubtext') || undefined}
            className="font-sans text-base text-on-primary-fixed mb-8 max-w-lg mx-auto leading-relaxed"
          >
            {page?.ctaSubtext ?? 'Get your price in minutes. Every job comes with this warranty, in writing.'}
          </p>
          <Link
