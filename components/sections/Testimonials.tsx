...
import type { SiteSettings, Testimonial } from '@/sanity/types'
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="mb-10 md:mb-14 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="font-sans text-base text-on-surface mt-4">{subheading}</p>
          </div>
          ...
          <div className="shrink-0 flex flex-col items-start sm:items-end gap-1">
            ...
            {googleProfileUrl ? (
              ...
              >
                {reviews.averageRating.toFixed(1)} · {reviews.totalCount} Reviews · Google →
              </a>
            ) : (
        ...
        <div
          className="flex gap-0 overflow-x-auto snap-x snap-mandatory -mx-4 px-4 md:grid md:grid-cols-3 md:overflow-x-visible md:mx-0 md:px-0 ghost-border"
        ...
        <p className="mt-4 text-center font-headline text-[0.8125rem] uppercase tracking-[0.2em] text-on-surface/80 md:hidden" aria-hidden="true">
          Swipe to read more
        </p>
...
function TestimonialCard({ testimonial: t, index }: { testimonial: Testimonial; index: number }) {
  return (
    <article
      className="snap-start shrink-0 w-[82vw] sm:w-[55vw] md:w-auto flex flex-col bg-surface-container-lowest ghost-border p-7 card-interactive animate-stagger-child"
    ...
    >
      ...
      <p className="font-sans text-base text-on-surface leading-relaxed flex-1 mb-6">{t.text}</p>
        ...
        <div className="flex items-center gap-3">
          {t.customerImage && (
            <div className="relative shrink-0 w-10 h-10 overflow-hidden rounded-full bg-on-surface/10">
              <Image src={t.customerImage} alt={t.name} fill className="object-cover" />
            </div>
          )}
          <div>
